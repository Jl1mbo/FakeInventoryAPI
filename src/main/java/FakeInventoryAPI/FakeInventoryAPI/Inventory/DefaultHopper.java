package FakeInventoryAPI.FakeInventoryAPI.Inventory;

import FakeInventoryAPI.FakeInventoryAPI.Utils.FakeChest;
...
public class DefaultHopper extends FakeChest {
    ...
    private static byte[] getNbt(BlockVector3 pos, String name) {
        CompoundTag tag = new CompoundTag().putString("id", BlockEntity.HOPPER).putInt("x", pos.x).putInt("y", pos.y).putInt("z", pos.z).putString("CustomName", name == null ? "Воронка" : name);
        try {
